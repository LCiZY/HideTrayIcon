...
VOID SetTrayIconVisable(HWND hWnd, std::vector<char*>& process_name_or_tooltips, bool visible, bool isHardDelete)
{
	printf("\n\n====================\n");
				...
				for (DWORD i = 0; i < dwButtonCount; i++) {
					...
					if (flag1 && flag2 && flag3) {
						...
						for (int i = 0; i < process_name_or_tooltips.size(); i++){
							if (contain(const_cast<char*>(tip.c_str()), process_name_or_tooltips.at(i)) ||
								contain(const_cast<char*>(pro_name.c_str()), process_name_or_tooltips.at(i))) {
								existFlag = true; 
								break; 
							}
						}
